...
//
//
//    func fetchUserPosts(client: SupabaseClient, supabaseManager: SupabaseManager, userId: UUID, type: String) async {
//        isLoading = true
...
//                        .execute()
//                    count = commentCountResponse.count ?? 0
//                    do {
...
//                }
//                let post = Post(id: p.id, user_id: p.user_id, image_path: url.absoluteString, caption: p.caption, created_at: p.created_at, type: p.type, commentCount: count, likeCount: lc, isLikedByCurrentUser: isLiked)
